...
/// Currently used as a helper in tests folder.
...
impl StartDate {
    pub fn new(year: Option<i32>, month: Option<u32>, date: u32) -> Self {
...
impl EndDate {
    pub fn new<T>(year: i32, month: u32, date: u32) -> EndOf<T> {
...
impl Frequency {
    ...
    pub fn monthly_by_date(start: StartDate, end: Option<EndOf<NumberMonths>>) -> Self {
        let current = Utc::now();
        let start_year = start.year.unwrap_or_else(|| current.year());
        let start_month = start.month.unwrap_or_else(|| current.month());

        let dt = NaiveDate::from_ymd_opt(start_year, start_month, start.date).unwrap();
